...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!historyData || !historyData.length) {
      ...
      const filename = `treated_drinks_${timestamp}.csv`;
    ...
    // 配列に変換して売上順にソート
    const drinkSummary = Object.values(drinkSummaryMap).sort((a, b) => b.total_sales - a.total_sales);
    ...
    // CSVデータの作成
    const csvRows = drinkSummary.map((drink: DrinkSummary) => [
      drink.cast_name,
      drink.product_name,
      drink.quantity.toString(),
      drink.total_sales.toString()
    ]);
    ...
    console.error('奢りドリンク内訳CSVエクスポートAPI: 予期せぬエラー:', error);
    return NextResponse.json(
