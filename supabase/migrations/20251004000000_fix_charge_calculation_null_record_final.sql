-- v_next_event_after_moveがNULLの場合のエラーを最終的に修正
...
DECLARE
  ...
  v_next_event_changed_at timestamptz := NULL; -- 次のイベントの時間を別の変数で保持
...
BEGIN
  ...
  IF v_last_move_charge_time IS NOT NULL THEN
    ...
    -- 席移動後の最初のイベントの時間を取得（移動先テーブルの席種情報）
    SELECT changed_at INTO v_next_event_changed_at
    FROM public.session_seat_events
    WHERE 
  ...
  -- v_next_event_changed_atを使用して次のイベントの時間をチェック
  IF v_has_move_charge AND v_next_event_changed_at IS NOT NULL AND 
     v_last_event_time = v_next_event_changed_at THEN
...
-- コメント追加
COMMENT ON FUNCTION public.fn_calc_total_charge(uuid) IS 'v_next_event_after_moveがNULLの場合のエラーを最終的に修正（2025/10/04）';
